...
| 26 | Additive features | **B** | Mini-cart drawer, live search suggestions, gallery lightbox and swipe, quick bids, Concept D's command palette | None exist in production. Each can be built without backend changes: `useCart()`; `sale-lots/?search=&status=live` (anonymous, 10 s server cache, anonymous throttle 200/min); client-only gallery; eligibility-based chips. | Confirm each addition with the client (B); then F. |
...
| 30 | Bid-activity visualisations (Concept D) | **API** | Price-history step chart, sparklines, bid velocity ("bids · last 10 min"), activity heat and a marketplace-wide live-bid ticker, all derived from the sample bid history | Bid history is paged at 8 per request (`sale-lots/{id}/bids/`); there is no cross-lot activity feed or aggregate endpoint. | Per-lot charts can page the history (F); velocity, heat and the marketplace ticker need an aggregate endpoint (API). |
