...
| **Strategy** | Map to `components/concept-a/pages/SellerPage.jsx` → `components/concept-a/seller/StoreHero.jsx`, `components/concept-a/seller/StoreFacts.jsx`, `components/concept-a/seller/StoreInventory.jsx`. Only `name`, `city`, `member_since`, the counts, `live_now` and `entitlements` exist. Tagline, description, cover, pickup address/hours and the "verified" badge have no source (G). Build after E.2. |
...
Each production-side statement below was verified in the reference source. For each gap: reconcile it before implementation, or approve it as a functional change.

| # | Area | Preview shows | Production (verified) | Reconcile by |
|---|---|---|---|---|
| 1 | Seller ratings, reviews, follow | Not shown | No model fields or endpoints exist (`marketplace`, `operations`, `accounts`). | Keep them out. |
| 2 | Seller profile content | Tagline, description, cover, monogram colour, pickup address and hours, "Verified warehouse" badge (`components/concept-a/seller/*`, `components/concept-a/detail/SellerCard.jsx`) | `GET marketplace/sellers/{code}/` returns only `code, name, city, member_since, active_auction_count, buy_now_count, live_now, entitlements`. `orders/pickup-locations/` requires authentication. There is no "verified" flag; public sellers are active, subscription-eligible warehouses. | Get content from a static map keyed by seller code, or drop it. Business sign-off on "verified" wording. |
| 3 | Seller directory | "Sellers" nav → `/seller`; `SellersFeature` lists all sellers | No public seller-list endpoint and no `/seller` index route. | Use a curated code list with per-code lookups, or get approval for an endpoint. |
| 4 | Location on cards and detail | Seller or warehouse city | The public serializer emits only `warehouse.code` to anonymous and `buyer` users (`backend-reference/marketplace/serializers.py:228-254`). `trade_buyer` is treated as an operator and gets city and region. The seller endpoint does expose `city`. Consequence: `sellerCodeForLot` can't see `warehouse_type`, so Main-warehouse lots show their real code instead of `KHAZNA`. | Take the city from the seller lookup, or get approval for a serializer change. Decide the `KHAZNA` display rule. |
| 5 | Finished auctions | Sold and ended lot pages (`AuctionOutcome`), sold cards, sold lots in browse | List, detail and by-slug return 404 once `end_time` passes (`filter_public_sale_lots`), including for the winner. Browse requests `status=live` only. | Show the outcome only to users already on the page, or via bid-eligibility and the account area. No public "sold" catalogue. |
| 6 | Upcoming lots and "Remind me" | An upcoming state with a "Remind me" action that adds the lot to the watchlist (`UpcomingBox`) | Scheduled lots are public, and `featured` includes them, but the current detail page has no upcoming UI. No start notification exists; watching only triggers "Auction activity" on bids, at most once per 15 min. | Build the upcoming state (the data exists). Relabel "Remind me" as Watch and drop the reminder promise. |
| 7 | Anti-snipe wording | "Bids in the last 5 minutes extend the auction by 5 minutes" | Backend defaults are a 300 s window and 300 s extension (`backend-reference/marketplace/models.py:148-150`). The current production UI says "2 minutes" (`AuctionDetailClient.jsx:2161`). | Render the wording from `anti_snipe_enabled`, `anti_snipe_window_seconds` and `anti_snipe_extend_seconds` in the detail payload. |
| 8 | First-bid minimum and quick bids | First bid = starting bid; quick-bid chips from there (timed and live) | REST requires `starting_bid + 0.01` for the first timed bid; the WebSocket reports `starting_bid`. Live auctions use `starting_bid`. Production has no quick-bid chips. | Base chips on bid-eligibility `minimum_next_bid` or `next_valid_bid`. Chips are an additive UI change and still go through confirm and REST. |
| 9 | Bidder states | `neutral, highest, outbid, won, lost` | `neutral, highest, outbid, winner, ended_outbid, ended` (`views.py` `auction_bidder_state`). | Map the states, and add an "ended, no bid" state. |
| 10 | Urgency and countdown | Urgent < 1 h, critical < 10 min; days shown | Production: urgent < 10 min, critical < 3 min (browse, auction, homepage); hours uncapped; live `m:ss`. | Pick one set of thresholds. Adopting days is display-only. |
| 11 | Live sequencing | Next lot starts automatically after an intermission; a late bid pushes the clock to 12 s; "going once / twice" | The operator starts each lot; there is no automatic advance and no automatic live anti-sniping (the operator may extend). | Show "Waiting for the next lot". Don't imply automatic extension. "Going once" may stay as a pure time cue. |
| 12 | Live schedule metadata | "Coming up" events with countdowns and a presenter name | Events have no scheduled-start or presenter field (`started_at`, `ended_at`, `created_at` only). `status_group=upcoming` lists staged events without a time. | Show "Starting soon" with no clock, and drop the presenter. |
| 13 | Reserve | No reserve UI; live `reserve_not_met` shown as not sold | The reserve price is deactivated system-wide (`views.py:1251`). Live `reserve_not_met` means no bids. | Never show "reserve" wording; label the outcome "Unsold". |
| 14 | Watch state | Watched state on cards; header watchlist drawer | Listings are anonymous with no per-user flag. The watchlist API allows at most 8 per page, and a `?sale_lot=` filter exists. | Detail page: check `watchlist/?sale_lot={id}`, which also fixes the never-loaded state. Card-level state needs several pages or an approved backend change. |
| 15 | Product content | Bilingual title and description, highlights, specs, condition note, lot number | Single-language `title` and `description`. Arabic only via free-form `extra_data.title_ar`/`description_ar`. The auction adapter hard-codes `specs: []`. No highlights or condition-note fields. Identity is SKU or slug. | Design must degrade gracefully when Arabic, specs or highlights are missing. |
| 16 | Browse contract | Default "All" tab; sorts recommended, newest, most bids; category slugs (`?category=bulk-pallets`); price 0–10,000 step 50 | Tabs auction, buy_now, live; sorts `ending`, `price_low`, `price_high`. The backend can order by `bid_count` and `created_at`, but has no "recommended". Categories are filtered by English name (`category_name`) with no slug filter. Price 0–100,000 step 500. | Keep the production URL contract (inbound links). Any new tab or sort needs approval and a semantic decision. |
| 17 | Paging | Numbered pages or "load more" | Infinite scroll mixed with numbered pages (browse), "load more" elsewhere. | Choose one model (functional decision). |
| 18 | Homepage sections | No Featured, Hot, ticker, country empty state or mobile ending-soon bar; adds a stats band and newsletter | Featured shows seller-requested placements after moderation (`featured_status=approved`, `is_featured_on_homepage`). Hot, ticker, empty state and mobile bar exist. There is no stats endpoint (About hard-codes other numbers) and no newsletter backend. | Keep Featured; decide on the rest. Remove or source the stats; remove the newsletter or approve a backend. |
| 19 | Header | No country switcher, no notification bell | Both exist and carry logic (country scoping; ticketed notification socket). | Add both to the approved header. |
| 20 | Onboarding copy | "Create your account with your email and confirm it with a one-time code" (already aligned in the preview data) | Email and password with an email OTP. Phone is optional and never verified. There is no password reset. | Keep the copy aligned; do not promise phone verification or password reset. |
| 21 | Payment and deposit claims | "mada, Visa, Mastercard" and the Khazna wallet (the current production footer also shows STC Pay); "deposit never charged unless you win"; "pay within 24 hours" | Moyasar defaults are `creditcard` with the mada, visa and mastercard networks (env-configurable; production values not verified). Timed-auction eligibility comes from wallet coverage (not debited) or a paid deposit; top-ups are real charges; wins are charged automatically from the wallet, then from a consented saved card. The 24 h window applies to timed wins only; live wins have no deadline. | Business and legal review of all payment and deposit copy. Show only the methods that are enabled. |
| 22 | Locale and routes | `/en\|ar/concept-x/…`; `/product/[slug]`, `/live-auction` | Locale lives in localStorage and the profile with fixed URLs; the routes are `/shop/[id]` (numeric) and `/live/[slug]`. The backend hard-codes several customer URLs (inventory A.3). | Port components and strings, not routing. |
| 23 | Theme, fonts and brand | Light/dark switch; next/font brand families; "Khazna" | The customer site has no dark mode (dashboard theme only on the auth pages). Only the riyal glyph font is loaded. The UI and metadata say "Khaznah" (52 occurrences), while the backend names the Main seller "Khazna" (`KHAZNA`). | Dark mode is new scope. Add fonts through `next/font`. Decide one brand spelling. |
| 24 | Images | Plain `<img>` with local srcsets | `next/image` with `remotePatterns` (S3, API host, `i.ytimg.com`), qualities 65/75, WebP. | Build `LotImage` on `next/image`. |
| 25 | Screens not designed | — | Cart, checkout, payment, account (9 tabs), login/register, static pages and 404 have no preview screens. | Extend the approved concept before phases 5–7. |
| 26 | Additive features | Mini-cart drawer, live search suggestions, gallery lightbox and swipe, quick bids | None exist in production. Each can be built without backend changes: `useCart()`; `sale-lots/?search=&status=live` (anonymous, 10 s server cache, anonymous throttle 200/min); client-only gallery; eligibility-based chips. | Confirm each addition with the client. |
| 27 | Existing broken or missing links | — | `/account/orders/{id}` (backend notification links) and `/orders/1` (checkout placeholder) have no route. `llms.txt` claims JSON-LD that doesn't exist. | Adding routes or JSON-LD is a functional change; approve separately. |
